...
After the exploration of words and sentiments, we want to further find out how those high frequencey vocabulary are related. To to this, we apply topic modeling(LDA). LDA is an unsupervised generative probabilistic model which, typically given the number of topics, allocates each word in a document to a specific topic. [**Click here**](EM_lda6.html) to see an interactive LDA result in which we select 6 topics. There is a detail description on how to read the graph in the page.
